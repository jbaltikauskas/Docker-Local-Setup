#!/usr/bin/env bash
# Idempotent toolchain install for the Local-DockerStack-Provisioner repo.
#
...
# The checked-in configs set INSTALL_ROOT_FOLDER to
# ..\Local-DockerStack-Provisioner--Installs, which resolves to a sibling of
# the repo checkout. On Linux that parent dir is not user-writable by default,
# so pre-create the install root owned by the user.
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_PARENT="$(dirname "$REPO_ROOT")"
INSTALL_ROOT="${REPO_PARENT%/}/Local-DockerStack-Provisioner--Installs"
